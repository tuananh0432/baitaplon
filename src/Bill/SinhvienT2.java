 ...
 * @author ASUS
 */
public class SinhvienT2  {
    Connection conn=null;
    PreparedStatement sttm=null;
public int add(Giangvien sv){
    try {
        String sSQL="INSERT INTO hi.giangvien2(ID, tênGV, malop, tuoi, gioitinh, diachi, sdt, Email) "+
                "values(?,?,?,?,?,?,?,?)";
        conn=DBconect.getDBcConnection();
        sttm=conn.prepareStatement(sSQL);
        sttm.setString(1, sv.getID());
        sttm.setString(2, sv.getTenGV());
...
public int Update(Giangvien sv){
    try {
        String sSQL="UPDATE hi.giangvien2 SET  tênGV = ?, malop = ?, tuoi = ?, gioitinh = ?, diachi = ?, sdt = ?, Email= ?  where ID=?" ;
        conn=DBconect.getDBcConnection();
        sttm=conn.prepareStatement(sSQL);
        sttm.setString(8, sv.getID());
        sttm.setString(1, sv.getTenGV());
...
public int delete(String ID){
    try {
        ...
        String sSQL="delete from hi.giangvien2 where ID =?";
        conn=DBconect.getDBcConnection();
        sttm=conn.prepareStatement(sSQL);
        sttm.setString(1, ID);
